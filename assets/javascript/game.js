...
$(document).ready(function() {
  ...
  $(".character").on("click", function() {
    if (heroChosen === false) {
      ...
      for (i = 0; i < characters.length; i++) {
        if (
          characters[i].name === $(this).attr("data-character") &&
          characters[i].isChosen === false &&
          enemyChosen === false
        ) {
          characters[i].isChosen = true;
  ...
  $("#attack").on("click", function() {
    if (
      ...
      selectedHero.health =
        selectedHero.health - selectedEnemy.counterAttackPoints;
      $("#heroHealth").html(selectedHero.health);
      ...
      if (selectedHero.health < 1) {
        ...
        $("#losses").html(loss);
        $("#instructions").html(
          "You lost! Click on the Restart button to play a new game!"
        );
        ...
        if (defeatedEnemies.length === 3) {
          ...
          $("#wins").html(win);
          $("#instructions").html(
            "You won! Click on the Restart button to play a new game!"
          );
          $("#attack").attr("hidden", true);
        }
      }
